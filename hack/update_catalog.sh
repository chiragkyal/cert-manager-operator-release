...
render_catalog_bundle()
{
	...
	# To manually update `package` from cert-manager-operator to openshift-cert-manager-operator
	sleep 120
...
if [[ "${REPLICATE_BUNDLE_FILE_IN_CATALOGS}" != "yes" && "${REPLICATE_BUNDLE_FILE_IN_CATALOGS}" != "no" ]]; then
    log_error "invalid value provided for \"REPLICATE_BUNDLE_FILE_IN_CATALOGS\", must be \"yes\" or \"no\""
    exit 1
fi

if [[ "${USE_MIGRATE_LEVEL_FLAG}" != "yes" && "${USE_MIGRATE_LEVEL_FLAG}" != "no" ]]; then
